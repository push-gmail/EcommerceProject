...
const getName = (value: Category | string | any) => {
  return typeof value === "object" && value?.name ? value.name : "-";
};

const getId = (value: Category | string | any) => {
...
export default function SubCategories() {
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [search, rowsPerPage, categoryFilter]);
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (editingId) {
        await backendApi.put(`/master/updateSubCategoryById/${editingId}`, form);
      } else {
        await backendApi.post("/master/create-sub-category", form);
      }
  ...
  const handleDeactivate = async (id: string) => {
    ...
    try {
      await backendApi.delete(`/master/delete-sub-categories/${id}`);
      await fetchInitialData();
    } catch (err: any) {
      setError(
        err.response?.data?.message || "Failed to deactivate sub category"
      );
    }
  };
  ...
  const handleViewImage = (image: string | undefined, name: string) => {
    ...
    if (!imageSrc) {
      setError("No image available for this sub category");
      return;
    }
  ...
  const filteredSubCategories = useMemo(() => {
    ...
    return subCategories.filter((item) => {
      ...
      const selectedCategoryId = String(categoryFilter || "");

      const parentName = getName(item.categoryId).toLowerCase();
  ...
  const showingTo = Math.min(endIndex, filteredSubCategories.length);
        ...
        <div className="flex flex-col justify-between gap-5 rounded-3xl border border-emerald-400/20 bg-white/10 p-6 shadow-[0_0_45px_rgba(16,185,129,0.16)] backdrop-blur-2xl md:flex-row md:items-end">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-400">
              Select a category and create grocery sub categories like Rice,
              Pulses, Bread, Milk, Chips and more.
            </p>
          </div>
          ...
          <div className="mb-5 flex flex-col justify-between gap-4 md:flex-row md:items-center">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-400">
                Master creates sub-category under selected grocery category.
              </p>
            </div>
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setForm((prev) => ({
                    ...prev,
                    status: e.target.value as SubCategoryForm["status"],
                  }))
                }
              >
            ...
            <div className="mt-5 rounded-3xl border border-emerald-400/20 bg-black/25 p-4">
              ...
              <div className="flex items-center gap-4">
                <img
                  ...
                  alt={form.name || "Sub category preview"}
                ...
                <p className="max-w-xl text-sm text-slate-400">
                  Image will be shown in the sub category list after save.
                </p>
              </div>
            </div>
          )}
        ...
        <div className="rounded-3xl border border-emerald-400/20 bg-white/10 p-6 shadow-[0_0_45px_rgba(16,185,129,0.14)] backdrop-blur-2xl">
          <div className="mb-5 flex flex-col justify-between gap-4 xl:flex-row xl:items-center">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-400">
                Manage all grocery sub categories.
              </p>
            </div>
            ...
            <div className="flex w-full flex-col gap-3 md:flex-row xl:w-auto xl:items-center">
              ...
              <div className="relative w-full md:w-96">
                <input
                  ...
                  placeholder="Search by category, sub-category, slug, status..."
                  value={search}
                  ...
                  <tr>
                    <td colSpan={5} className="p-10 text-center">
                      <div className="mx-auto max-w-sm rounded-3xl border border-emerald-400/20 bg-white/5 p-6">
                        ...
                        <p className="mt-1 text-sm text-slate-400">
                          Select category, write sub-category name and add your
                          first sub category.
                        </p>
                      </div>
                    </td>
          ...
          <div className="mt-4 flex items-center justify-end text-xs text-slate-500">
            <p className="text-emerald-300">
              Emerald Command Center · Sub Category Control
            </p>
          </div>
        </div>
      </div>
