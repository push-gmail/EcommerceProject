...
const statusDescriptions: Record<RequestStatus, string> = {
  pending: "Products submitted by you and waiting for master approval.",
  approved: "Products approved by master.",
  rejected: "Products rejected by master.",
  all: "All your product requests including pending, approved and rejected.",
};

const approvalStyle: Record<ApprovalStatus, string> = {
...
export default function MerchantRequests() {
  ...
  const fetchRequests = async () => {
    try {
      ...
      const res = await backendApi.get<ListResponse<MerchantProduct>>(
        `/merchant/product-requests?status=${currentStatus}&t=${Date.now()}`
      );
  ...
  const filteredProducts = useMemo(() => {
    ...
    return products.filter((product) => {
      return (
        ...
        product.quantityInPackets?.toLowerCase().includes(q)
      );
    });
  }, [products, search]);
  ...
  const goToStatus = (status: RequestStatus) => {
    navigate(`/merchant/requests/${status}`);
  };
          ...
          <div className="mb-5 flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-400">
                Your submitted products and approval status.
              </p>
            </div>
                  ...
                  <tr>
                    <td colSpan={10} className="p-10 text-center">
                      <div className="mx-auto max-w-sm rounded-3xl border border-emerald-400/20 bg-white/5 p-6">
                        ...
                        <p className="mt-1 text-sm text-slate-400">
                          Product requests will appear after you submit products.
                        </p>
                      </div>
                    </td>
                  </tr>
  ...
  return (
    <span
      className={`inline-flex items-center gap-2 rounded-full border px-3 py-1 text-xs font-black uppercase ${
        approvalStyle[status] || approvalStyle.pending
      }`}
    >
      {icon}
      {status}
